...
public class OpenflowpluginTestCommandConsumer implements CommandProvider {

    private static final Logger LOG = LoggerFactory.getLogger(OpenflowpluginTestCommandConsumer.class);
...
//    private final NodeErrorListener nodeErrorListener = new NodeErrorListenerLoggingImpl();
    ...
    public OpenflowpluginTestCommandConsumer(BundleContext ctx) {
        this.ctx = ctx;
    }
...
//        createTestFlow(createTestNode(null), null, null);
        System.out.println("Coming into the flow test!!!");
    }
    ...
    private static InstructionsBuilder createMeterInstructions() {
        ...
        InstructionsBuilder dropBuilder=createSentToControllerInstructions();//createDropInstructions();     
        instructions.add(dropBuilder.getInstruction().get(0));
    ...
    public void _removeMDFlow(CommandInterpreter ci) {
        ...
        NodeBuilder tn = createTestNode(getActualNode().getId().getValue());
        ...
        String pri=ci.nextArgument();
        if(pri != null){
            testFlow.setPriority(Integer.parseInt(pri));
        }
    ...
    public void _addMDFlow(CommandInterpreter ci) {
        ...
        String flowtype = ci.nextArgument();
        FlowBuilder tf;
   //     testNode=createTestNode(getActualNode().getId().getValue());
        if (flowtype.equals("fTM")) {
            tf = createtablemiss(tn, flowtype, ci.nextArgument());
        } else {
            tf = createTestFlow(tn, flowtype, ci.nextArgument());
    ...
    private void writeFlow(CommandInterpreter ci, FlowBuilder flow, NodeBuilder nodeBuilder) {
        ...
        for(Instruction ins : flow.getInstructions().getInstruction()){
            System.out.println("*******writeFlow:"+ins);
        }
    ...
    public Node getActualNode(){
        ...
        for(Node node : nodeList){
            ...
            if(fcn != null) return node;
        }
        return null;
    }
    
    public void _printcflow(CommandInterpreter ci){
        Node node=getActualNode();
        if(node==null){
            System.out.println("No actual node !!!");
            return;
        }
        InstanceIdentifier<FlowCapableNode> path1 = InstanceIdentifier.builder(Nodes.class)
                .child(Node.class, node.getKey()).augmentation(FlowCapableNode.class).build();
        final TypeSafeDataReader reader = TypeSafeDataReader.forReader(dataBrokerService);
        FlowCapableNode capNodes=reader.<FlowCapableNode>readConfigurationData(path1);
        if(capNodes==null){
            System.out.println("No actual flowcapable node !!!");
            return;
        }
        List<Table> tables=capNodes.getTable();
        for(Table table : tables){
            System.out.println("流表："+table.getKey().getId());
            List<Flow> flows=table.getFlow();
            for(Flow flow : flows){
                System.out.println(flow.getKey()+"  "+flow.getPriority()+"  "+flow.getInstructions());
            }
        }
    }
    public void _printoflow(CommandInterpreter ci){
        Node node=getActualNode();
        if(node==null){
            System.out.println("No actual node !!!");
            return;
        ...
        InstanceIdentifier<FlowCapableNode> path1 = InstanceIdentifier.builder(Nodes.class)
                .child(Node.class, node.getKey()).augmentation(FlowCapableNode.class).build();
        final TypeSafeDataReader reader = TypeSafeDataReader.forReader(dataBrokerService);
        FlowCapableNode capNodes=reader.<FlowCapableNode>readOperationalData(path1);
        if(capNodes==null){
            System.out.println("No actual flowcapable node !!!");
            return ;
        }
