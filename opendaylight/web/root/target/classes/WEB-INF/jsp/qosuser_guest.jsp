...
<input type="button" value="修改用户信息" onclick="document.getElementById('normalmodify').style.display='block';document.getElementById('fade').style.display='block'"><br>
...
<form method="post" name="modnormaluser" id="modnormaluser">
  ...
  <input type="button" value="提交" onclick="modnormal()">
</form>
</div>
</div>
...
function goPage(pageIndex) {
    ...
    for(var i=page;i<page+10;i++){
        if(i<count){
        var normal=userlist[i].userName+','+userlist[i].userID+','+userlist[i].userPassword+','+
        userlist[i].userLevelID+','+userlist[i].userDescription;
     //   var info=' <input type="radio" id="normal" name="normal" style='display:none' value='+normal+' > ';
        var radio=' <input type="radio" style="display:none" id="normal" name="normal" value='+normal+'> ';
        arrTr.push([userlist[i].userID,userlist[i].userName,
                    userlist[i].userLevelID,userlist[i].userDescription,radio]);
        }
    } 
